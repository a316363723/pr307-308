...
static void min_upd_run_cb(void* data);
static void spd_upd_run_cb(void* data);
static bool int_assign_value_cb(void* data);
static bool spd_assign_value_cb(void* data);
static bool min_assign_value_cb(void* data);

static void ctrl_btn_update_cb(void *data);
static void roller_value_changed_cb(uint32_t value);

static void page_candle_construct(void);
static void page_candle_destruct(void);
static void page_candle_time_upd_cb(uint32_t ms);
static void page_candle_event_cb(int event);
/*********************
 *  STATIC VATIABLES
 *********************/
static adj_btn_t btn_list[] = {
    ...
    ADJ_BTN_INITIALIZER(FREQUENCY_LEVEL_1, FREQUENCY_LEVEL_10, 1, FREQUENCY_LEVEL_10, "FRQ", "", 9, 59, -6,false, false,  frequence_get_value_str, spd_upd_run_cb, spd_assign_value_cb, NULL, NULL), 
    ADJ_BTN_INITIALIZER(UI_INTENSITY_MIN, UI_MIN_INETENSITY_MAX, UI_INTENSITY_STEP, UI_INTENSITY_VAL, "MIN", "", 9, 59, -6,false, false,  intensity_get_value_str, min_upd_run_cb, min_assign_value_cb, NULL, common_intensity_get_step), 
};

static lv_group_t *indev_group;
static struct db_fx_candle *p_candle_model;
static state_btn_t state_btn_dsc;
static light_mode_list_ext_t list_ext;
/*********************
 *  GLOBAL VATIABLES
 *********************/
page_t page_candle = PAGE_INITIALIZER("Candle",
                                PAGE_CANDLE, 
                                page_candle_construct, 
                                page_candle_destruct, 
                                page_candle_time_upd_cb,
                                page_candle_event_cb,                                
                                PAGE_LIGHT_EFFECT_MENU);
/*********************
 *  GLOBAL FUNCTIONS
 *********************/
void page_candle_init(lv_obj_t* parent)
{
	uint8_t hsmode;
    ...
    p_candle_model = (struct db_fx_candle*)screen_get_act_page_data();
    ...
    /* 最小亮度 */
    // lv_obj_t* min_cont = adj_btn_create(parent, LIGHT_EFFECT_COMMON_RADIUS, ADJ_BTN_TYPE_18);
    // lv_obj_set_user_data(min_cont, (void *)&btn_list[2]);
    // adj_btn_init(min_cont, &btn_list[2], 90);
    // lv_obj_add_event_cb(min_cont, adj_btn_event_cb, LV_EVENT_ALL, NULL);    
    // lv_obj_set_size(min_cont, INT_BTN_WIDTH, INT_BTN_HEIGHT);    
    // lv_obj_align_to(min_cont, int_cont, LV_ALIGN_OUT_RIGHT_TOP, DIVIDE_SPACE, 0);
    // lv_group_add_obj(indev_group, min_cont);
    ...
    lv_obj_add_flag(colors_cont, LV_OBJ_FLAG_EVENT_BUBBLE);
    ...
    lv_obj_set_size(colors_cont, CCT_RANGE_BTN_WIDTH, CCT_RANGE_BTN_HEIGHT);
    ...
    lv_label_set_text(name_label, Lang_GetStringByID(STRING_ID_CCT_RANGE));
    lv_obj_align(name_label, LV_ALIGN_LEFT_MID, 5, 0);    

    list_ext.bubble = true;
    list_ext.selected = (int8_t)p_candle_model->cct;
    list_ext.value_changed_cb = roller_value_changed_cb;
    list_ext.list = light_mode_list_create(colors_cont, &list_ext);

    lv_obj_set_size(list_ext.list, lv_pct(30), lv_pct(100));
    lv_obj_align(list_ext.list, LV_ALIGN_RIGHT_MID, -5, 0);
    light_mode_list_add_label(list_ext.list, Lang_GetStringByID(STRING_ID_WARMER));
    light_mode_list_add_label(list_ext.list, Lang_GetStringByID(STRING_ID_NATUAL));
    light_mode_list_add_label(list_ext.list, Lang_GetStringByID(STRING_ID_COOLER));
    ...
    lv_obj_set_size(spd_cont, FRQ_BTN_WIDTH, FRQ_BTN_HEIGHT);
    ...
    if (p_candle_model->state > state_btn_dsc.max) 
    {
        p_candle_model->state = state_btn_dsc.max;
    }

    state_btn_dsc.cur_index = p_candle_model->state;
...
static void page_candle_construct(void)
{
    ...
    light_effect_title_create(t_cont, Lang_GetStringByID(STRING_ID_CANDLE));
    ...
    page_candle_init(b_cont);
    
    uint8_t id = PAGE_CANDLE;
    uint8_t sub_id = 0;
...
static void page_candle_time_upd_cb(uint32_t ms)
{
    ...
    if (state_btn_dsc.cur_index != p_candle_model->state || force_upd)    //更新触发按键
    {
    ...
    {
        uint8_t selected = (uint8_t)list_ext.selected; 
        if (selected != p_candle_model->cct || force_upd)
        {            
            list_ext.selected = (int8_t)p_candle_model->cct;
...
static void page_candle_event_cb(int event)
{
	...
	uint8_t  hsmode ;
	data_center_read_config_data(SYS_CONFIG_HIGH_SPEED_MODE, &hsmode);
    data_center_read_light_data(LIGHT_MODE_FX_CANDLE, &candle);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&candle,  sizeof(struct db_fx_candle));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_candle_model, sizeof(struct db_fx_candle));            
            if (dest_crc32 != src_crc32)
            {
                if(hsmode == 0)data_center_write_light_data(LIGHT_MODE_FX_CANDLE, p_candle_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
            memcpy(p_candle_model, &candle, sizeof(struct db_fx_candle));
            page_set_invalid_type(PAGE_INVALID_TYPE_PORTION);
        }
        break;
		case EVENT_DATA_COMPULSION:
            data_center_write_light_data(LIGHT_MODE_FX_CANDLE, p_candle_model);
            break;
...
static void spd_upd_run_cb(void* data)
{
    ...
    p_candle_model->spd = (uint8_t)dsc->value;
}

static void int_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_candle_model->lightness = dsc->value;
}

static void min_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    //p_candle_model->min_int = dsc->value;
	(void)dsc;
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    //ASSIGN_VALUE_TO_BTN(p_candle_model->min_int);	
	(void)dsc;
	return false;
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_candle_model->lightness);
}

static bool spd_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_candle_model->spd);
//	(void)dsc;
//	return false;
}
...
static void ctrl_btn_update_cb(void *data)
{
    ...
    p_candle_model->state = (uint8_t)dsc->cur_index;   
	set_hs_fx_window();	
}

static void roller_value_changed_cb(uint32_t value)
{
    p_candle_model->cct = value;
}
