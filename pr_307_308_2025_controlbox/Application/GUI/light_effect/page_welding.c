...
#define MIN_BTN_WIDTH               136
#define MIN_BTN_HEIGHT              35
...
//static void refresh_min_btn(void* data, int16_t lightness);

static void page_welding_construct(void);
static void page_welding_destruct(void);
static void page_welding_time_upd_cb(uint32_t ms);
static void page_welding_event_cb(int event);
/*********************
 *  STATIC VATIABLES
 *********************/
static lv_group_t *indev_group;
static struct db_fx_welding_2 *p_welding_model;
static adj_btn_t btn_list[] = {
    ...
    ADJ_BTN_INITIALIZER(FREQUENCY_LEVEL_1, FREQUENCY_LEVEL_10, 1, FREQUENCY_LEVEL_10, "FRQ", "", 9, 59, -6, false, false,  frequence_get_value_str, frq_upd_run_cb, frq_assign_value_cb, NULL, NULL),                
    ADJ_BTN_INITIALIZER(0, 750, 1, 325, "MIN", "%", 9, 59, -6, false, false,  intensity_get_value_str, min_upd_run_cb, min_assign_value_cb, NULL, common_intensity_get_step),                
};
 ...
 *  GLOBAL VATIABLES
 *********************/
page_t page_welding = PAGE_INITIALIZER("Welding",
                                PAGE_WELDING, 
                                page_welding_construct, 
                                page_welding_destruct, 
                                page_welding_time_upd_cb,
                                page_welding_event_cb,                                
                                PAGE_LIGHT_EFFECT_MENU);
/*********************
 *  GLOBAL FUNCTIONS
 *********************/
void page_welding_init(lv_obj_t* parent)
{
	uint8_t hsmode;
    ...
    p_welding_model = (struct db_fx_welding_2*)screen_get_act_page_data();
    ...
    lv_roller_set_selected(lm_roller, p_welding_model->mode, LV_ANIM_OFF);
    ...
    lightmode_disp_panel_refresh(disp_panel, p_welding_model->mode, &p_welding_model->mode_arg); 

    roller_ext.obj = lm_roller;
    roller_ext.disp_panel = disp_panel;
    roller_ext.type = p_welding_model->mode;
    roller_ext.setting = &p_welding_model->mode_arg;
    ...
    lv_obj_set_size(min_cont, MIN_BTN_WIDTH, MIN_BTN_HEIGHT);
    lv_obj_align_to(min_cont, lm_roller, LV_ALIGN_OUT_BOTTOM_LEFT, 0, DIVIDE_SPACE);
    //refresh_min_btn(&btn_list[2], p_welding_model->lightness);
    ...
    lv_obj_align_to(frq_cont, min_cont, LV_ALIGN_OUT_BOTTOM_LEFT, 0, DIVIDE_SPACE);
    ...
    if (p_welding_model->state > state_btn_dsc.max)
    {
        p_welding_model->state = state_btn_dsc.max;
    }

    state_btn_dsc.cur_index = p_welding_model->state;
    ...
    lv_obj_align_to(state_btn, min_cont, LV_ALIGN_OUT_RIGHT_TOP, DIVIDE_SPACE, 0);
...
static void page_welding_construct(void)
{
    ...
    light_effect_title_create(t_cont, Lang_GetStringByID(STRING_ID_WELDING));
    ...
    page_welding_init(b_cont);
    
    uint8_t id = PAGE_WELDING;
    uint8_t sub_id = 0;
...
//    }
    ...
    if (state_btn_dsc.cur_index != p_welding_model->state || force_upd)    //更新触发按键
    {
    ...
    {
        if (roller_ext.type != p_welding_model->mode || force_upd)
        {
            roller_ext.type = p_welding_model->mode;    
            lv_roller_set_selected(roller_ext.obj, roller_ext.type, LV_ANIM_OFF);
        }
...
static void page_welding_event_cb(int event)
{
	...
	uint8_t  hsmode ;
	data_center_read_config_data(SYS_CONFIG_HIGH_SPEED_MODE, &hsmode);
    data_center_read_light_data(LIGHT_MODE_FX_WELDING_II, &welding);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&welding,  sizeof(struct db_fx_welding_2));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_welding_model, sizeof(struct db_fx_welding_2));            
            if (dest_crc32 != src_crc32)
            {
                if(hsmode == 0)data_center_write_light_data(LIGHT_MODE_FX_WELDING_II, p_welding_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
            memcpy(p_welding_model, &welding, sizeof(struct db_fx_welding_2));
...
static void int_upd_run_cb(void* data)
{
    ...
    p_welding_model->lightness = dsc->value;
    //refresh_min_btn(&btn_list[BTN_ID_MIN], p_welding_model->lightness);
}

static void min_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_welding_model->min_lightness = dsc->value;
}

static void frq_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_welding_model->frq = (uint8_t)dsc->value;
}

static bool int_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_welding_model->lightness);
}

static bool min_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_welding_model->min_lightness);
}

static bool frq_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_welding_model->frq);
}

static void ctrl_btn_update_cb(void *data)
{
    state_btn_t *dsc = (state_btn_t*)data;
    p_welding_model->state = (uint8_t)dsc->cur_index;
	set_hs_fx_window();	    
}
...
static void roller_value_change_cb(struct _light_mode_roller_ext* ext)
{
    ...
    p_welding_model->mode = (enum fx_mode)ext->type;
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    int32_t min = lightness * 80 / 100;
    
    dsc->max = min;
    if (dsc->value > dsc->max)
    {    
       dsc->value = dsc->max;
       p_welding_model->min = dsc->value;
       adj_btn_run_update_cb(data);
    }    
}
#endif
