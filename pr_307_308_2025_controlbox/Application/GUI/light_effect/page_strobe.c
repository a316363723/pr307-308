...
static struct db_fx_strobe_2 *p_strobe_model;
...
void page_strobe_init(lv_obj_t* parent)
{
    ...
    p_strobe_model = (struct db_fx_strobe_2 *)screen_get_act_page_data();
    ...
    lv_roller_set_selected(lm_roller, p_strobe_model->mode, LV_ANIM_OFF);
    ...
    lightmode_disp_panel_refresh(disp_panel, p_strobe_model->mode, &p_strobe_model->mode_arg); 

    roller_ext.obj = lm_roller;
    roller_ext.disp_panel = disp_panel;
    roller_ext.type = p_strobe_model->mode;
    roller_ext.setting = &p_strobe_model->mode_arg;
    ...
    if (p_strobe_model->state > state_btn_dsc.max)
    {
        p_strobe_model->state = state_btn_dsc.max;
    }

    state_btn_dsc.cur_index = p_strobe_model->state;
...
static void page_strobe_time_upd_cb(uint32_t ms)
{
    ...
    if (state_btn_dsc.cur_index != p_strobe_model->state || force_upd)    //更新触发按键
    {
...
static void page_strobe_event_cb(int event)
{
	...
	uint8_t  hsmode ;
	data_center_read_config_data(SYS_CONFIG_HIGH_SPEED_MODE, &hsmode);
    data_center_read_light_data(LIGHT_MODE_FX_STROBE_II, &strobe);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&strobe,  sizeof(struct db_fx_strobe_2));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_strobe_model, sizeof(struct db_fx_strobe_2));            
            if (dest_crc32 != src_crc32)
            {
                if(hsmode == 0)data_center_write_light_data(LIGHT_MODE_FX_STROBE_II, p_strobe_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
            memcpy(p_strobe_model, &strobe, sizeof(struct db_fx_strobe_2));
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_strobe_model->spd = (uint8_t)dsc->value;
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_strobe_model->spd);
}
