...
static void page_copcar_construct(void);
static void page_copcar_destruct(void);
static void page_copcar_time_upd_cb(uint32_t ms);
static void page_copcar_event_cb(int event);
/*********************
 *  STATIC VATIABLES
 *********************/
static lv_group_t *indev_group;
static struct db_fx_cop_car_2 *p_copcar_model;
 ...
 *  GLOBAL VATIABLES
 *********************/
page_t page_copcar = PAGE_INITIALIZER("Cop Car",
                                PAGE_COPCAR, 
                                page_copcar_construct, 
                                page_copcar_destruct, 
                                page_copcar_time_upd_cb,
                                page_copcar_event_cb,                                
                                PAGE_LIGHT_EFFECT_MENU);
/*********************
 *  GLOBAL FUNCTIONS
 *********************/
void page_copcar_init(lv_obj_t* parent)
{
	uint8_t hsmode;
    ...
    p_copcar_model = (struct db_fx_cop_car_2*)screen_get_act_page_data();
    ...
    lv_obj_set_style_radius(colors_cont, 5, 0);
    lv_obj_set_style_text_font(colors_cont, Font_ResouceGet(FONT_18), 0);
    ...
    lv_obj_set_style_radius(colors_cont, 5, LV_STATE_FOCUSED);
    ...
    lv_label_set_text(name_label, "Colors");
    lv_obj_align(name_label, LV_ALIGN_LEFT_MID, 5, 0);    

    lv_obj_t* roller = my_roller_create(colors_cont, lv_pct(23), lv_pct(100), "R\nB\nR+B\nB+W\nR+B+W");         
    roller_ext.obj = roller;
    roller_ext.value_changed_cb = roller_value_changed_cb;
    lv_roller_set_selected(roller, p_copcar_model->color, LV_ANIM_OFF);
    ...
    lv_obj_align_to(frq_cont, colors_cont, LV_ALIGN_OUT_BOTTOM_LEFT, 0, DIVIDE_SPACE);
    ...
    if (p_copcar_model->state > state_btn_dsc.max)
    {
        p_copcar_model->state = state_btn_dsc.max;
    }

    state_btn_dsc.cur_index = p_copcar_model->state;
...
static void page_copcar_construct(void)
{
    ...
    light_effect_title_create(t_cont, Lang_GetStringByID(STRING_ID_COP_CAR));
    ...
    page_copcar_init(b_cont);
    
    uint8_t id = PAGE_COPCAR;
    uint8_t sub_id = 0;
...
static void page_copcar_time_upd_cb(uint32_t ms)
{
    ...
    //更新触发按键
    if (state_btn_dsc.cur_index != p_copcar_model->state || force_upd)
    {
        if (p_copcar_model->state > state_btn_dsc.max)
        {
            p_copcar_model->state = state_btn_dsc.max;
    ...
    if (roller_ext.obj != NULL)
    {
        ...
        if (selected != p_copcar_model->color || force_upd)
        {            
            lv_roller_set_selected(roller_ext.obj, p_copcar_model->color, LV_ANIM_OFF);
        }
    }
...
static void page_copcar_event_cb(int event)
{
    struct db_fx_cop_car_2 copcar;
    uint32_t src_crc32, dest_crc32;
	uint8_t  hsmode ;
	data_center_read_config_data(SYS_CONFIG_HIGH_SPEED_MODE, &hsmode);
    data_center_read_light_data(LIGHT_MODE_FX_COP_CAR_II, &copcar);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&copcar,  sizeof(struct db_fx_cop_car_2));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_copcar_model, sizeof(struct db_fx_cop_car_2));            
            if (dest_crc32 != src_crc32)
            {
                if(hsmode == 0)data_center_write_light_data(LIGHT_MODE_FX_COP_CAR_II, p_copcar_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
            memcpy(p_copcar_model, &copcar, sizeof(struct db_fx_cop_car_2));
            page_set_invalid_type(PAGE_INVALID_TYPE_PORTION);
        }
        break;
		case EVENT_DATA_COMPULSION:
            data_center_write_light_data(LIGHT_MODE_FX_COP_CAR_II, p_copcar_model);
            break;
...
static void int_upd_run_cb(void* data)
{
    ...
    p_copcar_model->lightness = dsc->value;
}

static void frq_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_copcar_model->frq = (uint8_t)dsc->value;
}

static bool int_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_copcar_model->lightness);
}

static bool frq_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_copcar_model->frq);
}

static void ctrl_btn_update_cb(void *data)
{
    state_btn_t *dsc = (state_btn_t*)data;
    p_copcar_model->state = (uint8_t)dsc->cur_index;
	set_hs_fx_window();
}

static void roller_value_changed_cb(uint32_t value)
{
    p_copcar_model->color = value;
}
