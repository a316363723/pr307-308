...
static void page_pulsing_construct(void);
static void page_pulsing_destruct(void);
static void page_pulsing_time_upd_cb(uint32_t ms);
static void page_pulsing_event_cb(int event);
/*********************
 *  STATIC VATIABLES
 *********************/
static lv_group_t *indev_group;
static struct db_fx_pulsing_2 *p_pulsing_model;
static adj_btn_t btn_list[] = {
    ADJ_BTN_INITIALIZER(0, 1000, 1, 500, "I NT", "%", 9, 60, -6, false, false,  intensity_get_value_str, int_upd_run_cb, int_assign_value_cb, NULL, common_intensity_get_step),    
    ADJ_BTN_INITIALIZER(FREQUENCY_LEVEL_1, FREQUENCY_LEVEL_R, 1, FREQUENCY_LEVEL_R, "FRQ", "", 9, 60, -6, false, false,  frequence_get_value_str, frq_upd_run_cb, frq_assign_value_cb, NULL, NULL),                 
    ADJ_BTN_INITIALIZER(SPEED_LEVEL_1, SPEED_LEVEL_10, 1, SPEED_LEVEL_1, "SPD", "", 9, 60, -6, false, false,  speed_get_value_str, spd_upd_run_cb, spd_assign_value_cb, NULL, NULL), 
};
 ...
 *  GLOBAL VATIABLES
 *********************/
page_t page_pulsing = PAGE_INITIALIZER("Pulsing",
                                PAGE_PULSING, 
                                page_pulsing_construct, 
                                page_pulsing_destruct, 
                                page_pulsing_time_upd_cb,
                                page_pulsing_event_cb,
                                PAGE_LIGHT_EFFECT_MENU);
/*********************
 *  GLOBAL FUNCTIONS
 *********************/
void page_pulsing_init(lv_obj_t* parent)
{
	uint8_t hsmode;
    ...
    p_pulsing_model = (struct db_fx_pulsing_2*)screen_get_act_page_data();
    ...
    lv_roller_set_selected(lm_roller, p_pulsing_model->mode, LV_ANIM_OFF);
    ...
    lightmode_disp_panel_refresh(disp_panel, p_pulsing_model->mode, &p_pulsing_model->mode_arg); 

    roller_ext.obj = lm_roller;
    roller_ext.disp_panel = disp_panel;
    roller_ext.type = p_pulsing_model->mode;
    roller_ext.setting = &p_pulsing_model->mode_arg;
    ...
    if (p_pulsing_model->state > state_btn_dsc.max)
    {
        p_pulsing_model->state = state_btn_dsc.max;
    }
    
    state_btn_dsc.cur_index = p_pulsing_model->state;
...
static void page_pulsing_construct(void)
{
    ...
    light_effect_title_create(t_cont, Lang_GetStringByID(STRING_ID_PULSING));
    ...
    page_pulsing_init(b_cont);
    
    uint8_t id = PAGE_PULSING;
    uint8_t sub_id = 0;
...
static void page_pulsing_time_upd_cb(uint32_t ms)
{
    ...
    if (state_btn_dsc.cur_index != p_pulsing_model->state || force_upd)    //更新触发按键
    {
    ...
    {
        if (roller_ext.type != p_pulsing_model->mode || force_upd)
        {
            roller_ext.type = p_pulsing_model->mode;    
            lv_roller_set_selected(roller_ext.obj, roller_ext.type, LV_ANIM_OFF);
        }
...
static void page_pulsing_event_cb(int event)
{
	...
	uint8_t  hsmode ;
	data_center_read_config_data(SYS_CONFIG_HIGH_SPEED_MODE, &hsmode);
    data_center_read_light_data(LIGHT_MODE_FX_PULSING_II, &pulsing);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&pulsing,  sizeof(struct db_fx_pulsing_2));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_pulsing_model, sizeof(struct db_fx_pulsing_2));            
            if (dest_crc32 != src_crc32)
            {
                 if(hsmode == 0)data_center_write_light_data(LIGHT_MODE_FX_PULSING_II, p_pulsing_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
            memcpy(p_pulsing_model, &pulsing, sizeof(struct db_fx_pulsing_2));
...
static void int_upd_run_cb(void* data)
{
    ...
    p_pulsing_model->lightness = dsc->value;
}

static void spd_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_pulsing_model->spd = (uint8_t)dsc->value;
}

static void frq_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_pulsing_model->frq = (uint8_t)dsc->value;
}

static bool int_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_pulsing_model->lightness);
}

static bool spd_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_pulsing_model->spd);
}

static bool frq_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_pulsing_model->frq);
}

static void ctrl_btn_update_cb(void *data)
{
    state_btn_t *dsc = (state_btn_t*)data;
    p_pulsing_model->state = (uint8_t)dsc->cur_index; 
	set_hs_fx_window();		
}
...
static void roller_value_change_cb(struct _light_mode_roller_ext* ext)
{
    ...
    p_pulsing_model->mode = (enum fx_mode)ext->type;
}
#endif
