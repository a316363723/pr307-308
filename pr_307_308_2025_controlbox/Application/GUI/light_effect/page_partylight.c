...
#define SAT_BTN_WIDTH           149
#define SAT_BTN_HEIGHT          69
...
static void page_partylight_construct(void);
static void page_partylight_destruct(void);
static void page_partylight_time_upd_cb(uint32_t ms);
static void page_partylight_event_cb(int event);
/*********************
 *  STATIC VATIABLES
 *********************/
static adj_btn_t btn_list[] = {
    ...
    ADJ_BTN_INITIALIZER(0, 100, 1, 6, "SAT", "%", 9, 59, -6, false, false,  sat_get_value_str, sat_upd_run_cb, sat_assign_value_cb, NULL, NULL), 
    ADJ_BTN_INITIALIZER(SPEED_LEVEL_1, SPEED_LEVEL_10, 1, 6, "SPD", "", 9, 59, -6,false, false,  frequence_get_value_str, spd_upd_run_cb, spd_assign_value_cb, NULL, NULL),                 
};

static lv_group_t *indev_group;
static struct db_fx_party_lights  *p_partylight_model;
static state_btn_t state_btn_dsc;
/*********************
 *  GLOBAL VATIABLES
 *********************/
page_t page_partylight = PAGE_INITIALIZER("Party Light",
                                PAGE_PARTYLIGHT, 
                                page_partylight_construct, 
                                page_partylight_destruct, 
                                page_partylight_time_upd_cb,
                                page_partylight_event_cb,                                
                                PAGE_LIGHT_EFFECT_MENU);
/*********************
 *  GLOBAL FUNCTIONS
 *********************/
void page_partylight_init(lv_obj_t* parent)
{
	uint8_t hsmode;
    ...
    p_partylight_model = (struct db_fx_party_lights*)screen_get_act_page_data();
    ...
    lv_obj_align(int_cont, LV_ALIGN_TOP_LEFT, 10, 30);    

    lv_obj_t* sat_cont = adj_btn_create(parent, LIGHT_EFFECT_COMMON_RADIUS, ADJ_BTN_TYPE_18);
    lv_obj_set_user_data(sat_cont, (void *)&btn_list[1]);
    adj_btn_init(sat_cont, &btn_list[1], 90);
    lv_obj_add_event_cb(sat_cont, adj_btn_event_cb, LV_EVENT_ALL, NULL);
    lv_obj_set_size(sat_cont, SAT_BTN_WIDTH, SAT_BTN_HEIGHT);
    lv_obj_align_to(sat_cont, int_cont, LV_ALIGN_OUT_BOTTOM_LEFT, 0, DIVIDE_SPACE);
    ...
    adj_btn_init(spd_cont, &btn_list[2], 30);
    lv_obj_add_event_cb(spd_cont, adj_btn_event_cb, LV_EVENT_ALL, NULL);
    lv_obj_set_size(spd_cont, INT_BTN_WIDTH,  INT_BTN_HEIGHT);
    lv_obj_align_to(spd_cont, int_cont, LV_ALIGN_OUT_RIGHT_TOP, DIVIDE_SPACE, 0);
    ...
    if (p_partylight_model->state > state_btn_dsc.max)
    {
        p_partylight_model->state = state_btn_dsc.max;
    }

    state_btn_dsc.cur_index = p_partylight_model->state;
    state_btn_dsc.update_cb = ctrl_btn_update_cb;
    state_btn_dsc.widget_refresh_cb = state_btn_refresh;
    state_btn_dsc.img_to_top = 38;
    state_btn_dsc.img_gap = 5;
    ...
    lv_obj_align_to(state_btn, sat_cont, LV_ALIGN_OUT_RIGHT_TOP, DIVIDE_SPACE, 0);
...
static void page_partylight_construct(void)
{
    ...
    light_effect_title_create(t_cont, Lang_GetStringByID(STRING_ID_PARTY));
    ...
    page_partylight_init(b_cont);
    
    uint8_t id = PAGE_PARTYLIGHT;
    uint8_t sub_id = 0;
...
static void page_partylight_time_upd_cb(uint32_t ms)
{
    ...
    //更新触发按键
    if (state_btn_dsc.cur_index != p_partylight_model->state || force_upd)
    {
...
static void page_partylight_event_cb(int event)
{
    struct db_fx_party_lights partylight;
    uint32_t src_crc32, dest_crc32;
	uint8_t  hsmode ;
	data_center_read_config_data(SYS_CONFIG_HIGH_SPEED_MODE, &hsmode);
    data_center_read_light_data(LIGHT_MODE_FX_PARTY_LIGHTS, &partylight);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&partylight,  sizeof(struct db_fx_party_lights));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_partylight_model, sizeof(struct db_fx_party_lights));            
            if (dest_crc32 != src_crc32)
            {
                 if(hsmode == 0)data_center_write_light_data(LIGHT_MODE_FX_PARTY_LIGHTS, p_partylight_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
            memcpy(p_partylight_model, &partylight, sizeof(struct db_fx_party_lights));
            page_set_invalid_type(PAGE_INVALID_TYPE_PORTION);
        }
        break;
		case EVENT_DATA_COMPULSION:
            data_center_write_light_data(LIGHT_MODE_FX_PARTY_LIGHTS, p_partylight_model);
            break;
...
static void int_upd_run_cb(void* data)
{
    ...
    p_partylight_model->lightness = dsc->value;
}

static void sat_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_partylight_model->sat = dsc->value;
}

static void spd_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_partylight_model->spd = (uint8_t)dsc->value;
}

static bool int_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_partylight_model->lightness);
}

static bool sat_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_partylight_model->sat);
}

static bool spd_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_partylight_model->spd);
}

static void ctrl_btn_update_cb(void *data)
{
    state_btn_t *dsc = (state_btn_t*)data;
    p_partylight_model->state = (uint8_t)dsc->cur_index;
	set_hs_fx_window();	    
}
