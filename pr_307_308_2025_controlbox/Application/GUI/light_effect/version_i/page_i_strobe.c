...
static void page_strobe_construct(void);
static void page_strobe_destruct(void);
static void page_strobe_time_upd_cb(uint32_t ms);
static void page_strobe_event_cb(int event);
...
static struct db_fx_strobe *p_strobe_model;
 ...
 *  GLOBAL VATIABLES
 *********************/
page_t page_strobe = PAGE_INITIALIZER("Strobe",
                                PAGE_STROBE, 
                                page_strobe_construct, 
                                page_strobe_destruct, 
                                page_strobe_time_upd_cb,
                                page_strobe_event_cb,                                
                                PAGE_LIGHT_EFFECT_MENU);
/*********************
 *  GLOBAL FUNCTIONS
 *********************/
void page_strobe_init(lv_obj_t* parent)
{
	uint8_t hsmode;
    ...
    p_strobe_model = (struct db_fx_strobe *)screen_get_act_page_data();
    ...
    adj_int_widget_creat(int_panel, 70, 78, p_strobe_model->lightness, &btn_list[0]);
    ...
    adj_cct_widget_creat(cct_panel, 150, p_strobe_model->mode_arg.cct.cct, &btn_list[1], 0);
    ...
    effects_refresh_scale_img(cct_panel, p_strobe_model->mode_arg.cct.cct, UI_CCT_MIN, UI_CCT_MAX);
    ...
    adj_frq_widget_creat(frq_panel, 54, p_strobe_model->frq, &btn_list[2]);
    ...
    state_panel = adj_state_widget_creat(parent,&p_strobe_model->state, 288, 50, ctrl_btn_update_cb);
...
static void page_strobe_construct(void)
{
    ...
    light_effect_title_create(t_cont, Lang_GetStringByID(STRING_ID_STROBE));
    ...
    page_strobe_init(b_cont);
    
    uint8_t id = PAGE_STROBE;
    uint8_t sub_id = 0;
...
static void page_strobe_time_upd_cb(uint32_t ms)
{
    ...
    switch(type)
    {
        ...
        case PAGE_INVALID_TYPE_ALL:
        {
            ...
            memcpy(p_strobe_model, &strobe, sizeof(strobe));
    ...
    if (state_user_data->cur_index != p_strobe_model->state || force_upd)    //更新触发按键
    {
        bool focused = fucosed_obj == state_user_data->btn ? true : false;
        if (p_strobe_model->state > state_user_data->max)
        {
            p_strobe_model->state = state_user_data->max;
        }
                
        state_user_data->cur_index = p_strobe_model->state;
    ...
    {
        if (roller_ext.type != p_strobe_model->mode || force_upd)
        {
            roller_ext.type = p_strobe_model->mode;    
...
static void page_strobe_event_cb(int event)
{
    ...
    data_center_read_light_data(LIGHT_MODE_FX_STROBE, &strobe);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&strobe,  sizeof(struct db_fx_strobe));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_strobe_model, sizeof(struct db_fx_strobe));            
            if (dest_crc32 != src_crc32)
            {
                data_center_write_light_data(LIGHT_MODE_FX_STROBE, p_strobe_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
			...
			if(type == 1)  
				break;
            memcpy(p_strobe_model, &strobe, sizeof(struct db_fx_strobe));
            p_strobe_model->mode = FX_MODE_CCT;
...
static void int_upd_run_cb(void* data)
{
    ...
    p_strobe_model->lightness = dsc->value;
    p_strobe_model->mode = FX_MODE_CCT;
    page_event_send(EVENT_DATA_WRITE);
}
...
static void cct_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
	(void)dsc;
    p_strobe_model->mode_arg.cct.cct = dsc->value;
    p_strobe_model->mode = FX_MODE_CCT;
    effects_refresh_scale_img(cct_panel, p_strobe_model->mode_arg.cct.cct, UI_CCT_MIN, UI_CCT_MAX);
    page_event_send(EVENT_DATA_WRITE);
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_strobe_model->lightness);
}
...
static void ctrl_btn_update_cb(void *data)
{
	...
	if(hsmode == 0)
	{
		p_strobe_model->state = (uint8_t)dsc->cur_index; 
		page_event_send(EVENT_DATA_WRITE);
	}
	set_hs_fx_window();
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
	ASSIGN_VALUE_TO_BTN(p_strobe_model->mode_arg.cct.cct);
}

//static void roller_value_change_cb(struct _light_mode_roller_ext* ext)
//{
//    ext->type = (param_setting_type_t)lv_roller_get_selected(ext->obj);
//    p_strobe_model->mode = (enum fx_mode)ext->type;
//}

static void frq_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_strobe_model->frq = (uint8_t)dsc->value;
    p_strobe_model->mode = FX_MODE_CCT;
    page_event_send(EVENT_DATA_WRITE);
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_strobe_model->frq);
}
#endif
