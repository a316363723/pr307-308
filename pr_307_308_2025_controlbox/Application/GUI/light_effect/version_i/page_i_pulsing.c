...
static struct db_fx_pulsing *p_pulsing_model;
...
void page_pulsing_init(lv_obj_t* parent)
{
    ...
    p_pulsing_model = (struct db_fx_pulsing*)screen_get_act_page_data();
    ...
    adj_int_widget_creat(int_panel, 70, 78, p_pulsing_model->lightness, &btn_list[0]);
    ...
    adj_cct_widget_creat(cct_panel, 150, p_pulsing_model->mode_arg.cct.cct, &btn_list[1], 0);
    ...
    effects_refresh_scale_img(cct_panel, p_pulsing_model->mode_arg.cct.cct, UI_CCT_MIN, UI_CCT_MAX);
    ...
    adj_frq_widget_creat(frq_panel, 54, p_pulsing_model->frq, &btn_list[2]);
    ...
    state_panel = adj_state_widget_creat(parent,&p_pulsing_model->state, 288, 50, ctrl_btn_update_cb);
...
static void page_pulsing_time_upd_cb(uint32_t ms)
{
    ...
    switch(type)
    {
        ...
        case PAGE_INVALID_TYPE_ALL:
        {
            ...
            memcpy(p_pulsing_model, &pulsing, sizeof(pulsing));
    ...
    if (state_user_data->cur_index != p_pulsing_model->state || force_upd)    //更新触发按键
    {
        bool focused = fucosed_obj == state_user_data->btn ? true : false;      
        if (p_pulsing_model->state > state_user_data->max)
        {
            p_pulsing_model->state = state_user_data->max;
        }

        state_user_data->cur_index = p_pulsing_model->state;
...
static void page_pulsing_event_cb(int event)
{
    ...
    data_center_read_light_data(LIGHT_MODE_FX_PULSING, &pulsing);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&pulsing,  sizeof(struct db_fx_pulsing));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_pulsing_model, sizeof(struct db_fx_pulsing));            
            if (dest_crc32 != src_crc32)
            {
                data_center_write_light_data(LIGHT_MODE_FX_PULSING, p_pulsing_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
			...
			if(type == 1)  
				break;
            memcpy(p_pulsing_model, &pulsing, sizeof(struct db_fx_pulsing));
            p_pulsing_model->mode = FX_MODE_CCT;
...
static void cct_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
	(void)dsc;
    p_pulsing_model->mode_arg.cct.cct = dsc->value;
    p_pulsing_model->mode = FX_MODE_CCT;
    effects_refresh_scale_img(cct_panel, p_pulsing_model->mode_arg.cct.cct, UI_CCT_MIN, UI_CCT_MAX);
    page_event_send(EVENT_DATA_WRITE);
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
	ASSIGN_VALUE_TO_BTN(p_pulsing_model->mode_arg.cct.cct);
}
