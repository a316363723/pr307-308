...
void page_paparazzi_init(lv_obj_t* parent)
{
    ...
    adj_int_widget_creat(int_panel, 70, 78, p_paparazzi_model->lightness, &btn_list[0]);
    ...
    adj_cct_widget_creat(cct_panel, 150, p_paparazzi_model->cct, &btn_list[1], 0);
    ...
    effects_refresh_scale_img(cct_panel, p_paparazzi_model->cct, UI_CCT_MIN, UI_CCT_MAX);
//    lv_obj_t* img_panel = lv_obj_get_child(cct_panel, 1);
//    lv_img_set_angle(img_panel, angle_number);
//    lv_obj_set_pos(img_panel, s_arrow_cct_pos.star_x, s_arrow_cct_pos.star_y);
    ...
    adj_frq_widget_creat(frq_panel, 54, p_paparazzi_model->frq, &btn_list[2]);
    ...
    state_panel = adj_state_widget_creat(parent,&p_paparazzi_model->state, 288, 50, ctrl_btn_update_cb);
...
static void page_paparazzi_time_upd_cb(uint32_t ms)
{
    ...
    switch(type)
    {
        ...
        case PAGE_INVALID_TYPE_ALL:
        {
            ...
            memcpy(p_paparazzi_model, &paparazzi, sizeof(paparazzi));
    ...
    if (state_user_data->cur_index != p_paparazzi_model->state || force_upd)    //更新触发按键
    {
        bool focused = fucosed_obj == state_user_data->btn ? true : false;      
        if (p_paparazzi_model->state > state_user_data->max)
        {
            p_paparazzi_model->state = state_user_data->max;
        }
                
        state_user_data->cur_index = p_paparazzi_model->state;
