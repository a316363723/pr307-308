...
static struct db_fx_tv  *p_tv_model;
...
void page_tv_init(lv_obj_t* parent)
{
    ...
    p_tv_model = (struct db_fx_tv *)screen_get_act_page_data();
    ...
    adj_int_widget_creat(int_panel, 70, 78, p_tv_model->lightness, &btn_list[0]);
    ...
    adj_cct_widget_creat(cct_panel, 150, p_tv_model->cct, &btn_list[1], 0);
    ...
    adj_frq_widget_creat(frq_panel, 54, p_tv_model->frq, &btn_list[2]);
    ...
    state_panel = adj_state_widget_creat(parent,&p_tv_model->state, 288, 50, ctrl_btn_update_cb);
...
static void page_tv_time_upd_cb(uint32_t ms)
{
    ...
    switch(type)
    {
        ...
        case PAGE_INVALID_TYPE_ALL:
        {
            ...
            page_set_invalid_type(PAGE_INVALID_TYPE_IDLE);
            data_center_read_light_data(LIGHT_MODE_FX_TV, &tv); 
            memcpy(p_tv_model, &tv, sizeof(tv));
    ...
    if (state_user_data->cur_index != p_tv_model->state || force_upd)    //更新触发按键
    {
        bool focused = fucosed_obj == state_user_data->btn ? true : false;      
        if (p_tv_model->state > state_user_data->max)
        {
            p_tv_model->state = state_user_data->max;
        }
                
        state_user_data->cur_index = p_tv_model->state;
...
static void page_tv_event_cb(int event)
{
    ...
    data_center_read_light_data(LIGHT_MODE_FX_TV, &tv);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&tv,  sizeof(struct db_fx_tv));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_tv_model, sizeof(struct db_fx_tv));            
            if (dest_crc32 != src_crc32)
            {
                data_center_write_light_data(LIGHT_MODE_FX_TV, p_tv_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
			...
			if(type == 1)  
				break;
            memcpy(p_tv_model, &tv, sizeof(struct db_fx_tv));
...
static void cct_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
	(void)dsc;
    p_tv_model->cct = dsc->value;
    if(p_tv_model->cct > dsc->max)
        p_tv_model->cct = dsc->max;
    else if(p_tv_model->cct < dsc->min)
        p_tv_model->cct = dsc->min;
    for(uint8_t i =0 ;i < 3; i++)
    {
        lv_obj_set_style_text_color(cct_label_panel[i], lv_color_make(132,132,132), 0);
    }
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
	ASSIGN_VALUE_TO_BTN(p_tv_model->cct);
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_tv_model->frq = (uint8_t)dsc->value;
    page_event_send(EVENT_DATA_WRITE);
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_tv_model->frq);
}
#endif
