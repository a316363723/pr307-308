...
static void page_colorchase_construct(void);
static void page_colorchase_destruct(void);
static void page_colorchase_time_upd_cb(uint32_t ms);
static void page_colorchase_event_cb(int event);
/*********************
 *  STATIC VATIABLES
 *********************/
...
static struct db_fx_color_chase *p_colorchase_model;
static state_btn_t state_btn_dsc;
/*********************
 *  GLOBAL VATIABLES
 *********************/
page_t page_colorchase = PAGE_INITIALIZER("Color Chase",
                                PAGE_COLORCHASE, 
                                page_colorchase_construct, 
                                page_colorchase_destruct, 
                                page_colorchase_time_upd_cb,
                                page_colorchase_event_cb,                                
                                PAGE_LIGHT_EFFECT_MENU);
/*********************
 *  GLOBAL FUNCTIONS
 *********************/
void page_colorchase_init(lv_obj_t* parent)
{
	uint8_t hsmode;
    ...
    p_colorchase_model = (struct db_fx_color_chase*)screen_get_act_page_data();
    ...
    if (p_colorchase_model->state > state_btn_dsc.max) 
    {
        p_colorchase_model->state = state_btn_dsc.max;
    }

    state_btn_dsc.cur_index = p_colorchase_model->state;
    ...
    lv_obj_align_to(trigger_btn, sat_cont, LV_ALIGN_OUT_RIGHT_TOP, DIVIDE_SPACE, 0);
...
static void page_colorchase_construct(void)
{
    ...
    light_effect_title_create(t_cont, Lang_GetStringByID(STRING_ID_COLOR_CHASE));
    ...
    page_colorchase_init(b_cont);
    
    uint8_t id = PAGE_COLORCHASE;
    uint8_t sub_id = 0;
...
static void page_colorchase_time_upd_cb(uint32_t ms)
{
    ...
    //更新触发按键
    if (state_btn_dsc.cur_index != p_colorchase_model->state || force_upd)
    {
        if (p_colorchase_model->state > state_btn_dsc.max) 
        {
            p_colorchase_model->state = state_btn_dsc.max;
...
static void page_colorchase_event_cb(int event)
{
    struct db_fx_color_chase colorchase;
    uint32_t src_crc32, dest_crc32;
	uint8_t  hsmode ;
	data_center_read_config_data(SYS_CONFIG_HIGH_SPEED_MODE, &hsmode);
    data_center_read_light_data(LIGHT_MODE_FX_COLOR_CHASE, &colorchase);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&colorchase,  sizeof(struct db_fx_color_chase));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_colorchase_model, sizeof(struct db_fx_color_chase));            
            if (dest_crc32 != src_crc32)
            {
                if(hsmode == 0)data_center_write_light_data(LIGHT_MODE_FX_COLOR_CHASE, p_colorchase_model);
            }
        }
        break;
        case EVENT_DATA_PULL:
        {
            memcpy(p_colorchase_model, &colorchase, sizeof(struct db_fx_color_chase));
            page_set_invalid_type(PAGE_INVALID_TYPE_PORTION);
        }
        break;
		case EVENT_DATA_COMPULSION:
            data_center_write_light_data(LIGHT_MODE_FX_COLOR_CHASE, p_colorchase_model);
            break;
...
static void int_upd_run_cb(void* data)
{
    ...
    p_colorchase_model->lightness = dsc->value;
}

static void sat_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_colorchase_model->sat = (uint8_t)dsc->value;
}

static void spd_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_colorchase_model->spd = (uint8_t)dsc->value;
}

static bool int_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_colorchase_model->lightness);
}

static bool spd_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_colorchase_model->spd);
}

static bool sat_assign_value_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_colorchase_model->sat);
}

static void ctrl_btn_update_cb(void *data)
{
    state_btn_t *dsc = (state_btn_t*)data;
    p_colorchase_model->state = (uint8_t)dsc->cur_index; 
	set_hs_fx_window();
}
