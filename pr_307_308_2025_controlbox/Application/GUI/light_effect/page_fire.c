...
static adj_btn_t btn_list[] = {
    ...
    ADJ_BTN_INITIALIZER(UI_INTENSITY_MIN, UI_INTENSITY_MAX, UI_INTENSITY_STEP, UI_INTENSITY_VAL, "MIN", "", 9, 59, -6,false, false,  intensity_get_value_str, min_upd_run_cb, min_assign_value_cb, NULL, common_intensity_get_step), 
};

static lv_group_t *indev_group;
static struct db_fx_fire_2  *p_fire_model;
...
void page_fire_init(lv_obj_t* parent)
{
    ...
    p_fire_model = (struct db_fx_fire_2*)screen_get_act_page_data();
    ...
    btnlist_ext.cur_index = p_fire_model->mode;
    btnlist_ext.max = 1;
    btnlist_ext.min = 0;
    ...
    lightmode_disp_panel_limit_setting_init(disp_panel, p_fire_model->mode, &p_fire_model->mode_arg);
    
    btnlist_ext.obj = btnlist_obj;
    btnlist_ext.disp_panel = disp_panel;
    btnlist_ext.setting.limit = &p_fire_model->mode_arg;
    ...
    // lv_obj_align_to(min_cont, btnlist_obj, LV_ALIGN_OUT_BOTTOM_LEFT, 0, DIVIDE_SPACE);
    ...
    if (p_fire_model->state > state_btn_dsc.max)
    {
        p_fire_model->state = state_btn_dsc.max;
    }

    state_btn_dsc.cur_index = p_fire_model->state;
...
static void page_fire_time_upd_cb(uint32_t ms)
{
    ...
    if (state_btn_dsc.cur_index != p_fire_model->state || force_upd)    //更新触发按键
    {
    ...
    if (btnlist_ext.cur_index != p_fire_model->mode || force_upd)
    {
        btnlist_refresh(btnlist_ext.obj, p_fire_model->mode);
    }
	lightmode_disp_panel_limit_setting_init(btnlist_ext.disp_panel, p_fire_model->mode,&p_fire_model->mode_arg);
...
static void page_fire_event_cb(int event)
{
	...
	uint8_t  hsmode ;
	data_center_read_config_data(SYS_CONFIG_HIGH_SPEED_MODE, &hsmode);
    data_center_read_light_data(LIGHT_MODE_FX_FIRE_II, &fire);   
    dest_crc32 = ui_calc_checksum((uint8_t*)&fire,  sizeof(struct db_fx_fire_2));    
    switch (event)
    {
        case EVENT_DATA_WRITE:
        {
            src_crc32 = ui_calc_checksum((uint8_t*)p_fire_model, sizeof(struct db_fx_fire_2));            
            if (dest_crc32 != src_crc32)
            {
                if(hsmode == 0)data_center_write_light_data(LIGHT_MODE_FX_FIRE_II, p_fire_model);
            }
        }
        break;
		...
		{
			bool force_upd = 0;
			memcpy(p_fire_model, &fire, sizeof(struct db_fx_fire_2)); 
			state_btn_dsc.cur_index = 0; 
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    p_fire_model->spd = (uint8_t)dsc->value;
}
...
static void min_upd_run_cb(void* data)
{
    adj_btn_t* dsc = (adj_btn_t*)data;
	(void)dsc;
    //p_fire_model->min_int = dsc->value;
}
...
{
    adj_btn_t* dsc = (adj_btn_t*)data;
    ASSIGN_VALUE_TO_BTN(p_fire_model->spd);
}
...
static bool min_assign_value_cb(void* data)
{
	...
	(void)dsc;
	return false;
    //ASSIGN_VALUE_TO_BTN(p_fire_model->min_int);
}
...
static void btnlist_update_cb(void *data)
{
    ...
    p_fire_model->mode = (enum fx_mode)ext->cur_index;        
    lightmode_disp_panel_limit_setting_init(ext->disp_panel, p_fire_model->mode, &p_fire_model->mode_arg);
}

static void btnlist_release_cb(void* data)
{
    my_btnlist_ext_t* ext = (my_btnlist_ext_t*)data;            
    uint8_t page_id = ui_limit_setting_get_page(p_fire_model->mode);
	(void)ext;
    ui_set_limit_setting(p_fire_model->mode,  &p_fire_model->mode_arg); 
