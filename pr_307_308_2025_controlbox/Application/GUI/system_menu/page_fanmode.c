...
static void page_fanmode_time_upd_cb(uint32_t ms)
{
    ...
    uint8_t fan_mode = p_sys_menu_model->fan_mode;
    ...
    if ((p_sys_menu_model->fan_mode != fan_mode && btnmatrix_ext.obj != NULL) || force_upd)
    {            
        //取消当前按键的聚焦
...
static const char* get_fanmode_desc_str(uint8_t fanmode)
{
    ...
    switch(fanmode)
    {
		...
		{			
			...
			if(lang == LANG_TYPE_EN)
				#if PROJECT_TYPE==307
				str = "Silent fan mode will force the fan to lowest spinning and reduce the total output power to 550W";
				#elif PROJECT_TYPE==308
				str = "Silent fan mode will force the fan to lowest spinning and reduce the total output power to 550W";
				#endif
			else
				#if PROJECT_TYPE==307
				str = "静音风扇模式将降低风扇降至最低转速，并将总输出功率降低至 550W";
				#elif PROJECT_TYPE==308
				str = "静音风扇模式将降低风扇降至最低转速，并将总输出功率降低至 550W";
				#endif
		}
