...
typedef enum{
	...
	RS485_Cmd_Version2,   	//0x28
	RS485_Cmd_Factory_RGBWW_Crc,   	//0x29
	RS485_Cmd_Clear_Run_Time,   	//0x30
	RS485_Cmd_user_custom = 0xFD,
    RS485_Cmd_Null,
}rs485_cmd_enum; 
...
typedef struct{
	uint16_t    intensity;      /*亮度：<0-1000>*/
    uint16_t    r;      /*亮度：<0-1000>*/
    uint16_t    g;           /*光源类型：<0-255>*/
	uint16_t   	b; 
...
/*------------------------------------*/
typedef struct{
    uint8_t     type;                   /*温度类型 0/255->所有温度，1->COB温度，2->环境温度，3->PCB温度*/
    int16_t     value;                  /*温度值*/
...
}rs485_motor_move_arg_t;/*size: 35*/
...
typedef struct{
    ...
    uint32_t    reserva6;
    uint32_t    reserva7;
}rs485_motor_reset_arg_t;/*size: 35*/
...
typedef struct{
    ...
    uint32_t    reserva7;
    uint32_t    reserva8;
}rs485_motor_state_arg_t; /*size: 35*/
...
/*------------------------------------*/
typedef struct{
    uint32_t  product_num;      /*Product，产品序列，0-代表想读取的第1个产品版本*/
    uint8_t   hard_ver_major;   /*硬件主版本号 版本号定义：主.次.修订 如0x01 0x00 0x00 表示1.0.0 两位版本号可舍弃修订版本号*/
    uint8_t   hard_ver_minor;   /*硬件次版本号*/
    uint8_t   hard_ver_revision;/*硬件修订版本号*/
    uint8_t   soft_ver_major;   /*软件主版本号 版本号定义：主.次.修订 如0x01 0x00 0x00 表示1.0.0 两位版本号可舍弃修订版本号*/
...
typedef union{
	...
	rs485_user_custom_t         user_custom_t;
