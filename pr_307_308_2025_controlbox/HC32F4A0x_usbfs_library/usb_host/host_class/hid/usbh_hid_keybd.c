...
static void USR_KEYBRD_Init (void)
{
    ...
    Debug("> Use Keyboard to tape characters: \n\n");
    Debug("\n\n\n\n\n\n");
#endif
...
//	//TODO
	if(data == 0x0A)
	{
//		if( memcmp("6LY",&Idata,3) == 0 && ii == 11 && ( 64 < Idata[5] &&  Idata[5] < 91 ) )
//		{
            data_center_write_config_data_no_event(SYS_CONFIG_UUID, Idata);
			memset(&Idata, 0, 20);
//		}
		memset(&Idata,0,20);
		data_num = 0;
	...
	{
		//memset(&FrameFlag.ProInformation_Data, 0, 20);
		Idata[data_num] = data ;
		data_num ++ ;
	}
...
static void KEYBRD_Decode(uint8_t *pbuf)
{
    ...
    if ((pbuf[1] == KBD_LEFT_SHIFT) || (pbuf[1] == KBD_RIGHT_SHIFT))   //chen 扫码枪设备第二个字节才是
    {
        shift = TRUE;
    }
    else
