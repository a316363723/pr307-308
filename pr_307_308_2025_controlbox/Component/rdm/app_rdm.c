...
#if PROJECT_TYPE==308
Device_Info_TypeDef s_device_info_msg = {
    .Rdm_Protocol = 0x0100,   //RDM协议版本
    .device_model_id = 0x0100,
    .product_category = 0x01FF,
    .soft_ver_id = 0x01000001,
    .dmx512_footprint = 0x0001,
    .dmx512_personality = 0x0107,
    .dmx512_start_addr = 0x0001,
    .sub_device_count = 0x0000,
    .sensor_count = 0x0001,
};
...
#if PROJECT_TYPE==307
char s_device_lable[32] = "ES CS15";
#endif
#if PROJECT_TYPE==308
char s_device_lable[32] = "ES XT26";
#endif
char s_software_ver[32]= {0};
#define GETVER( x )    { 		\
							x[0] =  (SOFTWARE_VERSION / 16 ) + 0x30; \
							x[1] =  '.'; \
							x[2] =  SOFTWARE_VERSION % 16 + 0x30; \
						}

//下面这个参数是选择DMX模型的，AP-107 AP-179有，其他的灯只有一个模型，所以只需要保留一条
const Dmx_Profile_Msg_TypeDef s_dmx_profile[DMX_PROFILE_NUM] = {
#if PROJECT_TYPE==307
    {13, "1:CCT & RGB 8-bit"},
	{9, "2:CCT 8-bit"},
    {13, "3:CCT & HSI 8-bit"},
	...
	{15, "9:FX 8-bit"},
    {11, "10:WWRGB 8-bit"},
	{16, "11:Ultimate 8-bit"},
    {23, "12:CCT & RGB 16-bit"},
    {15, "13:CCT 16-bit"},
    {23, "14:CCT & HSI 16-bit"},
	{17, "15:RGB 16-bit"}, 
	{17, "16:Adv.HSI 16-bit"},
    {15, "17:GEL 16-bit"},
    {15, "18:XY 16-bit"},
	{14, "19:Source 16-bit"},
    {21, "20:FX 16-bit"},
    {19, "21:WWRGB 16-bit"},
	{22, "22:Ultimate 16-bit"},
#endif
#if PROJECT_TYPE==308
	{9, "1:CCT 8-bit"},
	{9, "2:FX 8-bit"},
...
{
	uint16_t cmd_byte = 0;
    RDM_Packet_TypeDef *rdm = (RDM_Packet_TypeDef*)data;
    RDM_Packet_TypeDef *rdm_tx = (RDM_Packet_TypeDef*)g_rdm_tx_buf;
//	if(Set == g_rdm_ctr.recv_flag)
//	{
//        g_rdm_ctr.recv_flag = Reset;
        //UID匹配
		if (0 == Get_Respond_Flag(rdm)) 										//is packet for us?
		{
            memcpy(g_rdm_tx_buf, data, sizeof(g_rdm_tx_buf));
...
//					memcpy(&rdm_tx->Data[2], DevID, 6);
				...
				case DMX_START_ADDRESS:							//start address...
                {
					...
					if (rdm->Cmd == SET_CMD)
					{
						...
						if(NULL != g_pRdmSetStartAddrCallback){
							...
							g_tUIAllData.sys_menu_model.dmx_addr = AdrBuf.u16;
						}
					}
					else
					{
				...
				case DEVICE_INFO:
                {
					...
					s_device_info_msg.dmx512_personality = (dmx_profile_sel + 1) * 256 + DMX_PROFILE_NUM;
                ...
                case DMX_PERSONALITY:
                {
                    ...
                    {
                        rdm_tx->PDLen = 0;
						if(rdm_tx->Data[0] < DMX_PROFILE_NUM+1 && rdm_tx->Data[0] > 0){
							if(NULL != g_pRdmSetProfileCallback){
								g_pRdmSetProfileCallback(rdm_tx->Data[0] - 1);
								g_tUIAllData.sys_menu_model.dmx_profile_index = rdm_tx->Data[0] - 1;
							}
						}
                    }
                    else
...
//		usart_dma_tx(p_data, j);
        wired_dmx_transmit_with_mab(p_data, j);
	}
}
/*****************************************************************************************
...
static void Send_Normal_Ack_Msg(RDM_Packet_TypeDef* p_rdm)
{
//    Dmx_TxPinSetGpioInit();
//    DMX_RS485_EN_Out = DMX_EN_TX;
//    
//    __disable_irq();
//    DMX_UART_TX_EN_OUT = 0;
//    delay_us(260);//200us
//    DMX_UART_TX_EN_OUT = 1;
//    delay_us(50);//40us
//    __enable_irq();
//    Dmx_TxPinSetUartInit();
//    Uart2Send((uint8_t*)p_rdm, p_rdm->Length + 2);
	
//	  usart_wire_dmx_tx((uint8_t*)p_rdm, p_rdm->Length + 2);
