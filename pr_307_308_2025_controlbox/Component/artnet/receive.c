  ...
  for (i = 0; i < 1; i++) {
    ...
    if (p->data.admx.universe == n->ports.out[i].port_addr &&
      ...
      if (ipA == 0 && ipB == 0) {
        ...
        port->timeA = xTaskGetTickCount();//millis();
        ...
        port->timeB = xTaskGetTickCount();//millis();
...
int handle_address(node n, artnet_packet p) {
  ...
  old_net = p->data.addr.NetSwitch;
   if (p->data.addr.NetSwitch == PROGRAM_DEFAULTS) {
    ...
    n->state.SubSwitch = p->data.addr.NetSwitch & ~PROGRAM_CHANGE_MASK;
    n->state.subnet_net_ctl = TRUE;
  }

  // check if subnet or net has actually changed
  if (old_subnet != n->state.subnet || old_net != n->state.SubSwitch) {
  ...
  for (i =0; i < ARTNET_MAX_PORTS; i++) {
    if (p->data.addr.swin[i] == PROGRAM_NO_CHANGE)  {
      ...
      n->ports.in[i].port_addr = n->ports.in[i].port_addr & 0xfff0 | (p->data.addr.swin[i] & ~PROGRAM_CHANGE_MASK);//_make_addr(n->state.subnet, p->data.addr.swin[i]);
  ...
  for (i =0; i < ARTNET_MAX_PORTS; i++) {
    if (p->data.addr.swout[i] == PROGRAM_NO_CHANGE) {
      ...
      n->ports.out[i].port_addr = n->ports.out[i].port_addr & 0xfff0 | (p->data.addr.swout[i] & ~PROGRAM_CHANGE_MASK);//_make_addr(n->state.subnet, p->data.addr.swout[i]);
      n->ports.in[i].port_net_ctl = TRUE;
      n->ports.out[i].port_enabled = TRUE;
    }
  }
  ...
  if (p->data.todreq.command == 0x00) {
    for (i=0; i < limit; i++) {
      for (j=0; j < ARTNET_MAX_PORTS; j++) {
        if ((n->ports.out[j].port_addr & 0x0f) == p->data.todreq.address[i] &&
...
int handle_tod_control(node n, artnet_packet p) {
  ...
  for (i=0; i < ARTNET_MAX_PORTS; i++) {
    if ((n->ports.out[i].port_addr & 0x0f) == p->data.todcontrol.address &&
        n->ports.out[i].port_enabled) {
...
int handle_firmware(node n, artnet_packet p) {
  ...
  if ( p->data.firmware.type == ARTNET_FIRMWARE_FIRMFIRST ||
    ...
    if (n->firmware.peer.s_addr == 0) {
      ...
      n->firmware.last_time = xTaskGetTickCount();//millis();
      ...
      memcpy(n->firmware.data, (const void *)p->data.firmware.data, block_length);
		//memcpy(n->firmware.data, &block_length, block_length);
    ...
    if (n->firmware.peer.s_addr == p->from.s_addr &&
      ...
      memcpy(n->firmware.data + offset, (const void*)p->data.firmware.data, block_length);
      n->firmware.bytes_current += block_length;
