...
int artnet_tx_tod_request(node n) {
  ...
  for (i=0; i < ARTNET_MAX_PORTS; i++) {
    if (n->ports.out[i].port_enabled) {
      todreq.data.todreq.address[todreq.data.todreq.adCount++] = n->ports.out[i].port_addr & 0x0f;
    }
  }
...
int artnet_tx_tod_data(node n, int id) {
  ...
  tod.data.toddata.address = n->ports.out[id].port_addr & 0x0f;
...
//      artnet_error("Attempting to send %d when the max is %d, very very bad...\n", data_len, max_len);
  ...
  if ((ret = artnet_net_send(n, &p))) {
    ...
    firm->last_time = xTaskGetTickCount();//millis();
    firm->expected_block++;
...
int artnet_tx_build_art_poll_reply(node n) {
  ...
  ar->verH = 0;
  ar->ver = 0;
  ar->subH = n->state.SubSwitch;//ar->subH = 0; //modify by hxsi
...
//  if(n->state
  ...
  for (i=0; i< ARTNET_MAX_PORTS; i++) {
    ...
    ar->swin[i] = n->ports.in[i].port_addr & 0x0f;
    ar->swout[i] = n->ports.out[i].port_addr & 0x0f;
  }
