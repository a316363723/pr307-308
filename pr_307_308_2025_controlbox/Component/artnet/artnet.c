...
//    artnet_error("%s : attempt to send on a disabled port (id:%i)", __FUNCTION__, port_id);
  ...
  if (n->state.bcast_limit == 0) {
    ...
    nodes = find_nodes_from_uni(&n->node_list,
                                (uint8_t)(port->port_addr & 0x000f),//port->port_addr,modify by hxsi
                                ips,
                                n->state.bcast_limit);
...
int artnet_send_address(artnet_node vn,
  ...
  if (n->state.node_type == ARTNET_SRV || n->state.node_type == ARTNET_RAW) {
    ...
    //p.data.addr.filler1 = 0; //modify by hxsi
...
int artnet_set_subnet_addr(artnet_node vn, uint8_t subnet) {
  ...
  if (!n->state.subnet_net_ctl && subnet != n->state.subnet) {
    ...
    for (i =0; i < ARTNET_MAX_PORTS; i++) {
      n->ports.in[i].port_addr = n->ports.in[i].port_addr & 0xff0f | ((n->state.subnet & LOW_NIBBLE) << 4);//((n->state.subnet & LOW_NIBBLE) << 4) | (n->ports.in[i].port_addr & LOW_NIBBLE); modify by hxsi
      // reset dmx sequence number
      n->ports.in[i].seq = 0;

      n->ports.out[i].port_addr = n->ports.out[i].port_addr & 0xff0f | ((n->state.subnet & LOW_NIBBLE) << 4);//((n->state.subnet & LOW_NIBBLE) << 4) | (n->ports.out[i].port_addr & LOW_NIBBLE); modify by hxsi
    }
...
void check_timeouts(node n) {
  time_t now = xTaskGetTickCount();//millis();
