...
static e131_addr_t e131_addr;
unsigned short g_sacn_universe;
/* Create a socket file descriptor suitable for E1.31 communication */
int e131_socket(void) {
  return socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);  //IPPROTO_UDP
}

/* Bind a socket file descriptor to a port number for E1.31 communication */
int e131_bind(int sockfd, const uint16_t port) {
//  e131_addr_t addr;
  memset(&e131_addr, 0, sizeof(e131_addr)); 
  e131_addr.sin_family = AF_INET;
  e131_addr.sin_addr.s_addr = htonl(INADDR_ANY);	//IP
  e131_addr.sin_port = htons(port);				//端口
  memset(e131_addr.sin_zero, 0, sizeof e131_addr.sin_zero);
  return bind(sockfd, (struct sockaddr *)&e131_addr, sizeof e131_addr);
}
...
int e131_multicast_join(int sockfd, const uint16_t universe) {
  ...
  struct ip_mreq mreq;			//239.255.0.0 | 1 // inet_addr("239.255.0.1");
  mreq.imr_multiaddr.s_addr =  htonl(0xefff0000 | universe);		//设置组播地址
  mreq.imr_interface.s_addr = htonl(INADDR_ANY);				//设置本地地址
  
  /* 注意，在调用setsockopt()函数之前，需要绑定到一个地址 */
  return setsockopt(sockfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof mreq);
}

int el31_clear_join(int sockfd, const uint16_t universe)
{
  if (universe < 1 || universe > 63999) {
  	errno = EINVAL;
  	return -1;
...
ssize_t e131_recv(int sockfd, e131_packet_t *packet) {
  ...
  addr_len = sizeof(e131_addr);
  memset(packet->raw, 0, sizeof(packet->raw)); 

  /* 没接受到数据进入阻塞 */
//  Cut = recv(sockfd, packet->raw, sizeof packet->raw, 0);
  Cut=recvfrom(sockfd,packet->raw,sizeof(packet->raw),0,(struct sockaddr *)&e131_addr, (uint32_t *)&addr_len);
  ////DEBUG("E1.31 Recv:%d,%d\n", *(packet->raw), Cut);
...
e131_error_t e131_pkt_validate(const e131_packet_t *packet) {
  ...
  unsigned short universe = packet->raw[113]<<8|packet->raw[114];
  
  if(g_sacn_universe != universe)
  {
    return E131_ERR_UNIVERSE;//york褳賱矛褌斩圆蔻隙
  }
  
  if (ntohs(packet->root.preamble_size) != _E131_PREAMBLE_SIZE)
  {
	  ////DEBUG("preamble_size=%d\n", ntohs(packet->root.preamble_size));
