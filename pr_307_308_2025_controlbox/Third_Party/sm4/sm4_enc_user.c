...
sm4_data_t* user_plaintext_encryption(unsigned char* str, unsigned short len)
{
	...
	unsigned char plaintext_data[200] = { 0 };
	unsigned short plaintext_len = (str[4]|str[3]) - str[2];//明文的长度
...
sm4_data_t* user_chiphertext_decryption(unsigned char* str, unsigned short len)
{
	...
	unsigned char chiphertext_data[200] = { 0 };
	unsigned short plaintext_len = (str[4] | str[3]) - str[2];//解密后明文的长度
	unsigned short star_len = str[2];
	unsigned short chipherption_star_add = 0;
	unsigned short chipherption_len_count = 0;
