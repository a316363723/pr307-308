...
static lv_obj_t * indev_obj_act_test = NULL;
...
static void indev_keypad_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    indev_obj_act_test = lv_group_get_focused(g);
    ...
    /*Pressing*/
        ...
        /*Long press repeated time has elapsed?*/
			...
			else if(data->key == LV_KEY_UP || data->key == LV_KEY_DOWN) {
...
lv_indev_state_t encode_key_last_state = 0;
/**
...
static void indev_encoder_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    encode_key_last_state = last_state;
    ...
    if(data->enc_diff != 0) {
        ...
        /*In navigate mode focus on the next/prev objects*/
            ...
            int32_t s;
            if(data->enc_diff < 0) {
                for(s = 0; s < -data->enc_diff; s+=2) lv_group_focus_prev(g);
            }
            else if(data->enc_diff > 0) {
                for(s = 0; s < data->enc_diff; s+=2) lv_group_focus_next(g);
            }
        }
    }
}
