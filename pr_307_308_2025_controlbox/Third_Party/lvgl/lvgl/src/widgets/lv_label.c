...
#define LV_LABEL_SCROLL_DELAY       1000
...
static void lv_label_refr_text(lv_obj_t * obj)
{
    ...
    if(label->long_mode == LV_LABEL_LONG_SCROLL) {
        ...
        //lv_anim_set_repeat_delay(&a, LV_LABEL_SCROLL_DELAY);
        lv_anim_set_repeat_delay(&a, a.playback_delay);//设置scroll的滚动方式，显示头端的时候暂停1s，显示尾端的时候显示暂停1s.
...
#else
            lv_anim_set_values(&a, 10, lv_area_get_width(&txt_coords) - size.x);
