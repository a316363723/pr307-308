...
int16_t fire3_rate_max[11] = {20,  40,  60,  80,  80,  80,  80,  80,  80,  80, 80};
int16_t fire3_rate_min[11] = {-20, -40, -60, -80, -80, -80, -80, -80, -80, -80, -80};
	
uint16_t fire3_ct_max[11] = {500, 400, 400, 400, 350, 300, 250, 200, 150, 100, 80};
uint16_t fire3_ct_min[11] = {200, 180, 160, 140, 120, 100, 80,  60,  40,  20, 10};
#endif /*(Fire_USE == 1)*/
...
void Fire_Init(Effect_Fire *fire, const Light_Effect* effect, Effect_PulseWightRGBWW *rgbww_pwm, uint8_t restart, uint8_t type)
{
	...
	{
		fire->min_work_time = fire3_ct_max[1] - (fire3_ct_max[1] - fire3_ct_min[10]) * (fire_arg->times - 60)/240.0f;
		fire->max_work_time = fire->min_work_time;
		fire->max_rate = fire3_rate_max[1] + (fire3_rate_max[10] - fire3_rate_max[1]) * (fire_arg->times - 60)/240.0f;
...
void IAmHere_Init(void *effect_mode, const Light_Effect* effect, Effect_PulseWightRGBWW *rgbww_pwm, uint8_t restart)
{
	...
	{
		Effect_CCT2PulseWight(4500, 0, 500, &rgbww_pwm[0]);
		__disable_irq();
