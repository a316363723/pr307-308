...
static void four_uCh2uLong(unsigned char* in, unsigned long* out);             //四字节转换成unsigned long
static void uLong2four_uCh(unsigned long in, unsigned char* out);              //unsigned long转换成四字节
static unsigned long move(unsigned long data, int length);          //左移，保留丢弃位放置尾部
static unsigned long func_key(unsigned long input);                 //先使用Sbox进行非线性变化，再将线性变换L置换为L'
static unsigned long func_data(unsigned long input);                //先使用Sbox进行非线性变化，再进行线性变换L
//static void print_hex(unsigned char* data, int len);                 //无符号字符数组转16进制打印
...
//4字节无符号数组转无符号long型
static void four_uCh2uLong(unsigned char* in, unsigned long* out)
{
	int i = 0;
	*out = 0;
	for (i = 0; i < 4; i++)
...
//无符号long型转4字节无符号数组
static  void uLong2four_uCh(unsigned long in, unsigned char* out)
{
	int i = 0;
...
//左移，保留丢弃位放置尾部
static  unsigned long move(unsigned long data, int length)
{
...
static  unsigned long func_key(unsigned long input)
{
	int i = 0;
...
static  unsigned long func_data(unsigned long input)
{
	int i = 0;
	unsigned long ulTmp = 0;
	unsigned char ucIndexList[4] = { 0 };
...
{
	 int i = 0, j = 0;
	 unsigned char p[50] = {0};      //定义一个50字节缓存区
