...
#define LOAD_CHUNK  (1)
    ...
    for (ii = step*20; ii < length; ii += this_write) {
...
//返回值:0,正常
//    其他,失败
uint8_t mpu_MPU6050_Dmp_Init(void)
{
	uint8_t res=0;
...
uint8_t Dmp_Load_Firmware(void)
{
	static uint16_t load_step = 0;
	uint8_t res=0;
...
//		if(res)return 8;    
		...
		if(res)return 9;
		gyro_int1_config();	
		g_gyroscope.gyroscope_init_state = 1;
	}
    return 0;
}

uint8_t mpu_6050_data_state = 0;
static float mpu6050_pitch,mpu6050_roll,mpu6050_yaw;
void Task_volt_mpu_init(void)
{
	static uint8_t last_pitch_90 = 0;
	static uint8_t last_roll_90 = 0;
	#if NULL
	uint8_t res = 0;
	...
	switch(state)
	{
		...
		case 3:
		{
			res=mpu_set_sample_rate(DEFAULT_MPU_HZ);//设置FIFO
			if(!res) state++;
		}break;
		case 4:
		{
			res=dmp_load_motion_driver_firmware();//设置FIFO
			if(!res) state++;
		}break;
		case 5:
		{
		...
		default:{
		#endif
		if( mpu_6050_data_state == 1 )
		{
			mpu_dmp_get_data(&mpu6050_pitch,&mpu6050_roll,&mpu6050_yaw);
			g_gyroscope.pitch_angle = (int16_t)mpu6050_pitch * (-1);
			g_gyroscope.roll_angle = (int16_t)mpu6050_roll;
			g_gyroscope.yaw_angle = (int16_t)mpu6050_yaw;
			if(((g_gyroscope.pitch_angle > 85 && g_gyroscope.pitch_angle < 95)
				|| (g_gyroscope.pitch_angle < -85 && g_gyroscope.pitch_angle > -95)) && last_roll_90 == 0)
			{
			...
			if(((g_gyroscope.roll_angle > 85 && g_gyroscope.roll_angle < 95)
				|| (g_gyroscope.roll_angle < -85 && g_gyroscope.roll_angle > -95)) && last_pitch_90 == 0)
			{
...
//		*pitch = atan2(-2 * q1 * q3 + 2 * q0* q2, -2 * q3 * q3 - 2 * q2* q2 + 1)* 57.3;	// pitch
//		*roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
//		*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
//		*pitch = atan2(-2 * q1 * q3 + 2 * q0* q2, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// pitch
//		*roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
//		*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
...
//pitch = atan2(2 * (q.0 * q.2 - q.3 * q.1), 1 - 2 * (q.2 * q.2 + q.3 * q.3))
//		*pitch = atan2(-2 * q1 * q3 + 2 * q0* q2, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// pitch
//		*roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;	// roll
//		*yaw   = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;	//yaw
