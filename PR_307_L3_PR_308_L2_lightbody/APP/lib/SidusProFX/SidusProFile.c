...
//flash pwm 文件快速检查
static bool SidusProFile_CFX_PwmFile_FastCheck(uint8_t Type, uint8_t Bank, char* name)
{
...
//    static CFX_Init_Sta_Type Init_Sta = CFX_INIT_STA_IDLE;
    ...
    uint8_t* FileEndTag = NULL;
		uint32_t FileEndTagAddr = 0;  //chen
    ...
    switch(Init_Sta)
    {
        ...
        case CFX_INIT_STA_READ_BANK_DATA://写缓存区
            ...
            {
				while((InternalAddr % 4) != 0 )   //chen    华大单片机FLASH 编程，扇区/全擦除地址末位必须以 4 对齐(末位地址为：0x0，0x4，0x8，0xC)，编程单位是 4bytes，
					++InternalAddr;
        ...
        case CFX_INIT_STA_CHECK_AFTER_WRITE:
            FileEndTagAddr = (SIDUSPRO_CFX_CACHE_ADDR + sizeof(CFX_PwmFileHead_Type) + CacheFile->FrameNum * sizeof(SidusPro_Pwm_Type));
						while((FileEndTagAddr % 4) != 0 )   //chen    华大单片机FLASH 编程，扇区/全擦除地址末位必须以 4 对齐(末位地址为：0x0，0x4，0x8，0xC)，编程单位是 4bytes，
							++FileEndTagAddr;		
						FileEndTag = (uint8_t*)(FileEndTagAddr);
            if(0 == memcmp(&CacheFile->Tag[0], CFX_FileTag, sizeof(CFX_FileTag)) && CacheFile->FxType == CacheType 
...
uint8_t get_sidus_file_state(void)
{
	return Init_Sta;
}
