...
- CFX光效文件搬移缓存函数：
将外部SPI Flash中对应Type和Bank光效文件缓存到片上ROM/RAM中自定义地址，返回值为缓存的片上ROM/RAM地址。在缓存完毕后并检验缓存文件无误后，回复App Ack信息，对应Command为 BT_CommandType_CFX_Ctrl，Ctrl类型为Init。



```c
uint32_t SidusPro_CFX_File_Cache(uint8_t Type, uint8_t Bank)
{
...
```c
bool SidusPro_PwmQueue_Create(uint32_t count, uint32_t size)
{
    //Todo
}
```
...
```c
bool SidusPro_PwmQueue_Put(const void * item)
{
    //Todo
}
```
...
```c
bool SidusPro_PwmQueue_Get(void * item)
{
    //Todo       
}
```
...
- pwm队列重置函数：
重置pwm队列为创建时的状态，即为清空队列数据。



```c
bool SidusPro_PwmQueue_Reset(void)
{
    //Todo    
}
```
...
```c
bool SidusPro_PwmQueue_Is_Full(void)
{
    //Todo    
}
```
...
一体式灯可以直接使用SidusProFile.c(需实现一些接口函数)，分离式灯体可参考以下信息接收和存储光效文件。
文件接收走GATT通道，具体可参照GATT通讯协议内容。
...
1. 系统上电，快速校验光效文件信息并生记录本机光效文件存储信息（Type对应Bank下有无光效文件），只看头尾不计算校验值(所有光效都是2Min的情况下计算所有文件校验值时间过长)
1. GATT接收完一个完整光效后置位当Bank存储信息
1. 软件擦除对应Bank后清零当前Bank信息
...
CFX光效在运行的时候需要从外部SPI flash读取文件缓存到mcu片上Rom/Ram中，app控制时，还需要回复缓存完毕ack信息。所以需要实现接口中的最后一个函数。当缓存地址为片上ROM时，为了保护片上Flash寿命，在重复缓存同一光效文件时不应擦除当前缓存文件，应在判断外部SPI Flash中对应文件与当前缓存区文件是否一致后再做擦除动作。文件一致性判断条件为FxType相同、FxBank相同、FrameNum相同、PwmDataCrc相同，这几项与外部SPI Flash中文件相同且缓存区文件crc计算校验通过时，可不重复擦除进行缓存。
